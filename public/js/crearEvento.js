        ...
        function switchPreviewTab(tab) {
            ...
            document.getElementById(tab === 'card' ? 'cardView' : 'listView').classList.add('active');
        }
        ...
        document.getElementById('event_date').addEventListener('change', function(e) {
            if (e.target.value) {
                ...
                document.getElementById('previewDate').textContent = 'Fecha no especificada';
        ...
        document.getElementById('description').addEventListener('input', function(e) {
            document.getElementById('previewDescription').textContent = e.target.value || 'La descripción del evento aparecerá aquí...';
            document.getElementById('charCount').textContent = e.target.value.length;
        });

        document.getElementById('max_attendees').addEventListener('input', function(e) {
            const capacityDiv = document.getElementById('previewCapacityDiv');
            if (e.target.value) {
                document.getElementById('previewCapacity').textContent = e.target.value;
        ...
        document.getElementById('price').addEventListener('input', function(e) {
            const badge = document.getElementById('previewBadge');
            if (e.target.value) {
                badge.innerHTML = `<i class="fas fa-dollar-sign"></i> ${parseFloat(e.target.value).toFixed(2)}`;
            } else {
                badge.innerHTML = '<i class="fas fa-dollar-sign"></i> DE PAGO';
            }
        });
        ...
        document.getElementById('imageInput').addEventListener('change', function(e) {
            ...
            if (file) {
                ...
                reader.onload = function(e) {
                    ...
                    if (paymentType === 'pago') {
                        if (price) {
                            newBadge.innerHTML = `<i class="fas fa-dollar-sign"></i> ${parseFloat(price).toFixed(2)}`;
                        } else {
                            newBadge.innerHTML = '<i class="fas fa-dollar-sign"></i> DE PAGO';
                        }
        ...
        function removeImage() {
            ...
            if (paymentType === 'pago') {
                badgeHTML = price ? `<i class="fas fa-dollar-sign"></i> ${parseFloat(price).toFixed(2)}` : '<i class="fas fa-dollar-sign"></i> DE PAGO';
            ...
            document.getElementById('previewListImage').innerHTML = '<i class="fas fa-calendar-alt"></i>';
        }
        ...
        async function submitForm(status) {
            ...
            const button = status === 'borrador' ? document.getElementById('saveAsDraftBtn') : document.getElementById('publishBtn');
            const originalText = button.innerHTML;

            // Deshabilitar botones
            document.getElementById('saveAsDraftBtn').disabled = true;
            document.getElementById('publishBtn').disabled = true;
            button.innerHTML = '<i class="fas fa-spinner fa-spin"></i> ' + (status === 'borrador' ? 'Guardando...' : 'Publicando...');
